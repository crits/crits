...
class DashboardViewTests(SimpleTestCase):
    ...
    def testDashboard(self):
        response = views.dashboard(self.req)
        self.assertEqual(response.status_code, 200)
        self.assertTrue("id='dashboardWrapper'" in response.content)


class CoreViewsTests(SimpleTestCase):
    """
    Some miscellaneous tests for views in core/views.py
    These are simple tests just verifying 200 responses
    """
    ...
    def setUp(self):
        ...
        # We use administrator to avoid redirects on restricted pages
    ...
    def testAjaxPOST200(self):
        post_funcs = {
            'toggle_favorite': {
                'params': {'type': 'Indicator', 'id': '1234'},
                'args': {},
            },
            'favorites': {
                'params': {},
        ...
        for k, v in post_funcs.items():
            url = reverse('crits.core.views.%s' % k, kwargs=v['args'])
            self.req = self.factory.post(url, v['params'])
            self.req.user = self.user
            response = getattr(views, k)(self.req, **v['args'])
            self.assertEqual(response.status_code, 200)
    ...
    def testGet302(self):
        ...
        for k, v in get_funcs.items():
            url = reverse('crits.core.views.%s' % k, kwargs=v)
            self.req = self.factory.get(url)
            self.req.user = self.user
            response = getattr(views, k)(self.req, **v)
            self.assertEqual(response.status_code, 302)

    def testDialogs(self):
        get_funcs = [
            {'params': {'dialog': 'backdoor_add'}},
            {'params': {'dialog': 'exploit_add'}},
            {'params': {'dialog': 'action_add'}},
            {'params': {'dialog': 'indicator-blob'}},
            {'params': {'dialog': 'new-campaign'}},
            {'params': {'dialog': 'new-certificate'}},
            {'params': {'dialog': 'new-domain'}},
            {'params': {'dialog': 'new-email-eml'}},
            {'params': {'dialog': 'new-email-fields'}},
            {'params': {'dialog': 'new-email-indicator'}},
            {'params': {'dialog': 'new-email-outlook'}},
            {'params': {'dialog': 'new-email-raw'}},
            {'params': {'dialog': 'new-email-yaml'}},
            {'params': {'dialog': 'new-event'}},
            {'params': {'dialog': 'new-indicator-csv'}},
            {'params': {'dialog': 'new-indicator'}},
            {'params': {'dialog': 'new-inline-comment'}},
            {'params': {'dialog': 'new-ip'}},
            {'params': {'dialog': 'new-object-indicator'}},
            {'params': {'dialog': 'new-pcap'}},
            {'params': {'dialog': 'new-raw-data-file'}},
            {'params': {'dialog': 'new-raw-data'}},
            {'params': {'dialog': 'new-sample'}},
            {'params': {'dialog': 'new-standards'}},
            {'params': {'dialog': 'new-target'}},
            {'params': {'dialog': 'raw_data_type_add'}},
            {'params': {'dialog': 'releasability-add'}},
            {'params': {'dialog': 'shortcut_help'}},
            {'params': {'dialog': 'source-add'}},
            {'params': {'dialog': 'source_create'}},
            {'params': {'dialog': 'ticket'}},
            {'params': {'dialog': 'upload_tlds'}},
            {'params': {'dialog': 'user_role'}},
        ]
        for k in get_funcs:
            url = reverse('crits.core.views.%s' % 'get_dialog')
            self.req = self.factory.get(url, k['params'])
            self.req.user = self.user
            response = getattr(views, 'get_dialog')(self.req)
            self.assertEqual(response.status_code, 200)
            self.assertTrue('<span>Error!</span>' not in response.content)
            self.assertTrue('Error 500' not in response.content)
            self.assertTrue('Error 404' not in response.content)

    def testGet200(self):
        # I use 'inline' for the jTable views because we already render
        # base.html in other views and it cuts down on rendering time
        # by not including it
        get_funcs = {
            'about': {},
            'help': {},
            'profile': {'args': {'user': TUSER_NAME}},
            'favorites_list': {'args': {'ctype': 'Indicator', 'option': 'inline'}},
            'items_listing': {'args': {'itype': 'Backdoor', 'option': 'inline'}},
            'audit_listing': {'args': {'option': 'inline'}},
            'item_editor': {},
            'users_listing': {},
            'dashboard': {},
            'bucket_list': {},
            'timeline': {'args': {'data_type': 'dns'}},
        }
        ...
        for k, v in get_funcs.items():
            ...
            if 'args' not in v:
                v['args']= {}
            url = reverse('crits.core.views.%s' % k, kwargs=v['args'])
            self.req = self.factory.get(url, v['params'])
            self.req.user = self.user
            response = getattr(views, k)(self.req, **v['args'])
            self.assertEqual(response.status_code, 200)
            self.assertTrue('<span>Error!</span>' not in response.content)
